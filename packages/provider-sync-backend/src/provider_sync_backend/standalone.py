...
def _read_config(path: str | Path | None) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise ValueError("provider sync config must be a JSON object")
    return value
...
def main() -> None:
    ...
    host = os.environ.get("PROVIDER_SYNC_HOST") or "127.0.0.1"
    port = int(os.environ.get("PROVIDER_SYNC_PORT") or "8765")
    uvicorn.run("provider_sync_backend.standalone:app", host=host, port=port)
